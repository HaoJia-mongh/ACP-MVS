...
from .module import *
from .update import BasicUpdateBlockDepth, CoarsestUpdateBlockDepth
...
class DepthNet(nn.Module):
    def __init__(self):
        ...
        self.cwf_corr_stem = BasicConv(32, 1, is_3d=True, kernel_size=3, stride=1, padding=1)
        self.cwf_feature_att = FeatureAtt(1, 64)
    ...
    def forward(self, context, features, proj_matrices, depth_values, num_depth, cost_regularization):
        ...
        for src_fea, src_proj in zip(src_features, src_projs):
            ...
            if self.training:
                warped_volume = (warped_volume - ref_volume) ** 2
                reweight = self.cwf_corr_stem(warped_volume)
                reweight = self.cwf_feature_att(reweight, context.detach())
                volume_sum = volume_sum + reweight * warped_volume
            else:
                warped_volume = (warped_volume - ref_volume).pow_(2)
                reweight = self.cwf_corr_stem(warped_volume)
                reweight = self.cwf_feature_att(reweight, context.detach())
                volume_sum += reweight * warped_volume
            del warped_volume
        del ref_volume
...
class ACP_MVS(nn.Module):
    def __init__(self, args, depth_interals_ratio=[4,2,1], stage_channel=True):
        ...
        self.update_block_depth1 = CoarsestUpdateBlockDepth(hidden_dim=self.hdim_stage[0], cost_dim=self.cost_dim_stage[0]*self.CostNum,
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        for stage_idx in range(self.num_stage):
            ...
            hidden_d, inp_d1 = torch.split(cnet_depth_stage, [self.hdim_stage[stage_idx], self.cdim_stage[stage_idx]], dim=1)
            if stage_idx == 0:
                inp_d2 = inp_d1
                ref_feature2 = ref_feature

            current_hidden_d = torch.tanh(hidden_d)

            inp_d = torch.relu(inp_d1)
            ...
            if stage_idx == 0:
                ...
                init_depth = self.depthnet(inp_d1, features_stage, proj_matrices_stage, depth_values=depth_range_samples,
