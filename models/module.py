...
class ConvBnReLU3D1(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, pad=1):
        super(ConvBnReLU3D1, self).__init__()
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size, stride=stride, padding=pad, bias=False)
        self.bn = nn.BatchNorm3d(out_channels)

    def forward(self, x):
        return F.relu(self.bn(self.conv(x)), inplace=True).squeeze(1)


def convBnReLU3D(in_channels, out_channels, kernel_size=3, stride=1, pad=1):
    return ConvBnReLU3D1(in_channels, out_channels, kernel_size=3, stride=1, pad=1)
