...
#filter
parser.add_argument('--conf', type=float, default=0.35, help='prob confidence')
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src, ref_depth_max, ref_depth_min):
    ...
    mask = np.logical_and(dist < 0.135, depth_diff < 0.125)
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        os.makedirs(os.path.join(out_folder, "depth_est_averaged"), exist_ok=True)
        depth_est_averaged = (sum(all_srcview_depth_ests) + ref_depth_est) / (geo_mask_sum + 1)
        save_pfm(os.path.join(out_folder, "depth_est_averaged/{:0>8}_depth_est_averaged.pfm".format(ref_view)), depth_est_averaged.astype("float32"))
        ...
        #print("depth_est_averaged", depth_est_averaged.shape)
        final_mask_tensor = torch.from_numpy(final_mask)
        depth_est_averaged_tensor = torch.from_numpy(depth_est_averaged)
        depth_est_averaged_filter = depth_est_averaged_tensor.masked_fill(~final_mask_tensor, -1e9).numpy()
        #print("depth_est", depth_est_averagedg.masked_fill(~final_maskg, -1e9).shape)
        print("valid_points", valid_points.mean())
        x, y, depth = x[valid_points], y[valid_points], depth_est_averaged[valid_points]
        os.makedirs(os.path.join(out_folder, "depth_valid_points"), exist_ok=True)
        #print("depth", depth.shape)
        save_pfm(os.path.join(out_folder, "depth_valid_points/{:0>8}_depth_valid_points.pfm".format(ref_view)), depth_est_averaged_filter.astype("float32"))
        if num_stage == 1:
