...
from models import *
from utils import *
from datasets.data_io import read_pfm, save_pfm, save_png
...
def save_scene_depth(testlist):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            print('Memory usage:', torch.cuda.max_memory_allocated() / 1024 / 1024, 'M')
            torch.cuda.synchronize()
            end_time = time.time()
            ...
            for filename, cam, img, depth_est, photometric_confidence, depth_max_, depth_min_ in zip(filenames, cams, imgs, outputs["depth"][-1], outputs["photometric_confidence"], depth_max, depth_min):
                ...
                #plt.imsave(feat_att_filename, feat_att, cmap='gray')
